...
for video in videos:
  ...
  dirs = []
  dirs.append('./data/train/'+video)
  dirs.append('./data/valid/'+video)
  dirs.append('./data/test/'+video)
...
def process(image):
  ...
  for contour in contours:
    ...
    if area > 150000:
      ...
      scale = 1.02 # cropping margin, 1 == no margin
      W = rect[1][0]
      H = rect[1][1]
...
for filename in filenames:
  if (count % 5)==0:  
    ...
    filename = filename.replace('dataset', 'data/train')
    print(filename)
    cv2.imwrite(filename, process(image))
  elif (count % 37) == 0:
    image = cv2.imread(filename, cv2.IMREAD_COLOR)
    filename = filename.replace('dataset', 'data/valid')
    print(filename)
    cv2.imwrite(filename, process(image))
  elif (count % 59) == 0:
    image = cv2.imread(filename, cv2.IMREAD_COLOR)
    filename = filename.replace('dataset', 'data/test')
    print(filename)
    cv2.imwrite(filename, process(image))
  count = count + 1
