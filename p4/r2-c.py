...
def getLABMask():
  ...
  lower = (130, 110, 130)
  upper = (255, 255, 255)
  X = loadImages(I="testImages", color=cv2.COLOR_BGR2LAB)
  LABMask = X
  LABImage = X
  X = np.asarray(X)
  for i in range(0, len(X)):
...
duration = end - start
calcAccuracy(Y, mask)
np.save('results/lim-BGRMask-2.npy', mask)
print('{:.3f} s'.format(duration))
print ('\nNaive YCrCb Filter')
print ('='*20)

start = time.time()
X, mask, skin = getYCrCbMask()
Y = loadImages(I="testGTs")
...
duration = end - start
calcAccuracy(Y, mask)
np.save('results/lim-YCbCrMask-2.npy', mask)
print ('{:.3f} s'.format(duration))
